    ...
    # -------------------------------
    # Updated Encoder to return outputs
    # -------------------------------
    class Encoder(nn.Module):
        ...
        def forward(self, src, src_lens):
            ...
            return output, hidden  # output: (batch, src_len, hidden_dim), hidden: (num_layers, batch, hidden_dim)
    ...
    class Decoder(nn.Module):
        def __init__(self, output_dim, emb_dim, hidden_dim, num_layers, cell_type, dropout=0.0, attention=None):
            ...
            rnn_class = getattr(nn, cell_type)
            self.rnn = rnn_class(emb_dim + hidden_dim, hidden_dim, num_layers, batch_first=True, dropout=dropout if num_layers > 1 else 0.0)
            self.fc_out = nn.Linear(hidden_dim * 2, output_dim)
            self.attention = attention

        def forward(self, input_char, hidden, encoder_outputs, src_mask):
            embedded = self.embedding(input_char.unsqueeze(1))  # (batch, 1, emb_dim)
            if isinstance(hidden, tuple):  # LSTM case
                h = hidden[0][-1]  # (batch, hidden_dim)
            else:
                h = hidden[-1]     # GRU case

            attn_weights = self.attention(h, encoder_outputs, src_mask)  # (batch, src_len)
    ...
    class Seq2Seq(nn.Module):
        ...
        def create_src_mask(self, src):
            return (src != 0).to(src.device)  # (batch, src_len)
    ...
    # Training Utilities
    # -------------------------------
    def calculate_accuracy(preds, tgt):
        preds = preds.argmax(dim=2)
        correct, total = 0, 0
        for pred_seq, tgt_seq in zip(preds, tgt):
            for p, t in zip(pred_seq[1:], tgt_seq[1:]):
                if t.item() == 0:
                    break
    ...
    def train_model(config=None):
        with wandb.init(config=config,project="da6401_assignment_3", entity="cs24m048-iit-madras"):
            ...
            decoder = Decoder(len(base_dataset.target_vocab), config.embed_dim, config.hidden_dim, config.num_layers, config.cell_type, config.dropout , attention=attention).to(device)
            ...
            for epoch in range(1, config.epochs + 1):
                ...
                for src, tgt, _, _ in train_loader:
                    ...
                    train_acc += calculate_accuracy(output, tgt)
